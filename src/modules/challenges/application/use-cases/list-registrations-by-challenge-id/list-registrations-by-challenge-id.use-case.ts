...
@Injectable()
export class ListRegistrationsByChallengeIdUseCase {
  constructor(
  ...
  public async exec(challengeUuid: string) {
    const bucket = this.configService.get<string>('storage.repository');
    const registrations =
      await this.registrationsService.findReportByChallengeId(challengeUuid);
